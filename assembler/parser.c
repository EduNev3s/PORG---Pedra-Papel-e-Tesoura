...
int CarregaPrograma(char *nome)
{
  in = fopen(nome, "r");
  if (in == NULL)
    parser_Abort("Nao foi possivel carregar o arquivo de programa.");

  progr_buffer = (char *)malloc((MAX_BUFF_SIZE + 1) * sizeof(char));

  if (progr_buffer == NULL)
    parser_Abort("Memoria insuficiente para buffer do programa!");
...
void parser_SkipUntil(char c)
{
  while (Look != c)
  {
    parser_GetChar();
    if (Look == EOF)
      parser_Match(c); /* Match sempre falha, entao da erro se chegar no EOF */
  }
  parser_Match(c);
  parser_SkipWhite();
  parser_SkipComment();
...
char *parser_GetItem_s(void)
{
  ...
  int i = 0;
  if (!parser_IsAlNum(Look))
    parser_Expected("Label, Instrucao ou Registrador");
  while (parser_IsAlNum(Look))
  {
    Token[i] = Look;
    parser_GetChar();
    i++;
    if (i > LABEL_MAX_SIZE - 1)
      parser_Expected("Menos caracteres");
  }
  Token[i] = '\0';
  parser_SkipWhite();
  parser_SkipComment();
...
char *parser_GetNum_s(void)
{
  ...
  int i = 0;
  if (!parser_IsDigit(Look))
    parser_Expected("Numero");
